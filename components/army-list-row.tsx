...
export default function ArmyListRow(army) {
  ...
  const primaryPointsFor = isNaN(army.PrimaryPointsFor)
    ? 0
    : army.PrimaryPointsFor;
  const primaryPointsAgainst = isNaN(army.PrimaryPointsAgainst)
    ? 0
    : army.PrimaryPointsAgainst;
  const secondaryPointsFor = isNaN(army.SecondaryPointsFor)
    ? 0
    : army.SecondaryPointsFor;
  const secondaryPointsAgainst = isNaN(army.SecondaryPointsAgainst)
    ? 0
  ...
  return (
    <>
      <tr onClick={() => handleRowClick(army.id)} className="cursor-pointer">
        <td>{army.Name}</td>
        ...
        <td className="text-center">
          {Math.round(
            ((primaryPointsFor + secondaryPointsFor) / army.Played) * 10
          ) / 10}
        </td>
        ...
        <td className="text-center">
          {primaryPointsFor +
            secondaryPointsFor -
            primaryPointsAgainst -
            secondaryPointsAgainst}
        </td>
        <td className="text-center">
          {Math.round((army.Won / army.Played) * 1000) / 10 + "%"}
        </td>
        {/*
        <td className="text-center">
